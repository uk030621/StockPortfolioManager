...
const COLLECTION_USSTOCK_NAME = "LWJ_asia_stocks";
...
export async function GET(req) {
  ...
  try {
    // Check if the symbol is '^GDAXI' for direct, independent retrieval of DAX index data
    if (symbol === "^N225") {
      //console.log("Fetching ^GDAXI index data...");
      ...
      try {
        ...
        //console.log("Received ^GDAXI data:", JSON.stringify(data.chart.result[0].meta, null, 2));

        // Retrieve DAX index values directly
        const pricePerShare =
          data?.chart?.result?.[0]?.meta?.regularMarketPrice;
        if (pricePerShare !== undefined) {
          //console.log(`DAX Index (^GDAXI) value: ${indexValue}`);
          return NextResponse.json({
            symbol,
            pricePerShare: pricePerShare.toFixed(2),
          });
        } else {
          //console.error("Index data missing for ^GDAXI.");
          return NextResponse.json(
            { error: `Index data missing for ^GDAXI` },
            { status: 500 }
          );
        }
      } catch (error) {
        //console.error("Error fetching ^GDAXI data:", error.message || error);
        return NextResponse.json(
          {
            error:
              "Failed to retrieve ^GDAXI index data due to timeout or network error",
          },
          { status: 500 }
        );
      }
